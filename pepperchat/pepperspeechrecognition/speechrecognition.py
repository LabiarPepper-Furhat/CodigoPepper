...
RECORDING_DURATION = 10     # segundos, tiempo máximo de grabación, también valor predeterminado para startRecording(), Google Speech API solo acepta hasta aproximadamente 10-15 segundos
LOOKAHEAD_DURATION = 1.0    # segundos, para el modo de detección automática: cantidad de segundos antes de que se active el umbral que se incluirá en la solicitud
IDLE_RELEASE_TIME = 2.0     # segundos, para el modo de detección automática: tiempo de inactividad (RMS por debajo del umbral) después del cual dejamos de grabar y reconocer
HOLD_TIME = 3.0             # segundos, tiempo mínimo de grabación después de haber comenzado a grabar (detección automática)
SAMPLE_RATE  = 48000         # Hz, tenga cuidado al cambiar esto, tanto Google como Naoqi tienen requisitos!

CALIBRATION_DURATION = 4    # segundos, período de tiempo durante el cual se realiza la calibración (sumando los valores RMS y calculando la media)
CALIBRATION_THRESHOLD_FACTOR = 1.5  # factor por el cual se multiplica la media calculada de RMS para determinar el umbral de detección automática (después de la calibración)

DEFAULT_LANGUAGE = "en-us"  # etiqueta de idioma RFC5646, por ejemplo, "en-us", "de-de", "fr-fr",... <http://stackoverflow.com/a/14302134>

WRITE_WAV_FILE = False      # escribe el audio grabado en "out.wav" antes de enviarlo a Google. destinado a fines de depuración
PRINT_RMS = False           # imprime el valor RMS calculado en la consola, útil para establecer el umbral

PREBUFFER_WHEN_STOP = False # Rellena el pre-buffer con las últimas muestras al detener la grabación. ADVERTENCIA: tiene problemas de rendimiento!
